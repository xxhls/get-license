...
// 获取所有的 License 路径
const licensesPath = fs.readdirSync(`${process.cwd()}/src/templates/`).filter(license => license !== 'index.ts');
// 获取所有的 License 名称
const licenses = licensesPath.map(license => path.parse(license).name).map(license => license.toLowerCase());

// 若要新增 License，1 需要在这里添加 License 枚举 & 对应的生成器
enum License {
  MIT,
  Apache2,
}
